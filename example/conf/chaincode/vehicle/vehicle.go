...
const (
	VEHICLE_STATUS_SELLING int32 = iota
	VEHICLE_STATUS_SALED
	VEHICLE_STATUS_LEASED
)
...
func createVehicle(stub shim.ChaincodeStubInterface, args []string) (string, error) {
	...
	brand := strings.TrimSpace(args[1])
	price, _ := strconv.ParseFloat(strings.TrimSpace(args[2]), 64)
	createTime, _ := strconv.ParseInt(strings.TrimSpace(args[3]), 10, 64)
...
func createLease(stub shim.ChaincodeStubInterface, args []string) (string, error) {
	...
	id := strings.TrimSpace(args[0])
	vehicleId := strings.TrimSpace(args[1])
	userId := strings.TrimSpace(args[2])

	resp, err := stub.GetState(vehicleId)
	if err != nil {
		return "", err
	}
	err = json.Unmarshal(resp, vehicle)
	if err != nil {
		log.Printf("Get vehicle error:%s", err.Error())
		return "", err
	}

	l := Lease{
		ObjectType: OBJECTTYPE_LEASE,
		Id:         id,
		VehicleId:  vehicleId,
		UserId:     userId,
	}
...
func findLease(stub shim.ChaincodeStubInterface, args []string) (string, error) {
	...
	res, err := stub.GetPrivateData(PRIVATE_COLLECTION_LEASE, id)
	if err != nil {
...
func main() {
	...
	err := shim.Start(new(VehicleSharing))
	if err != nil {
