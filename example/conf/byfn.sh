...
function networkUp() {
  ...
  if [ "${CERTIFICATE_AUTHORITIES}" == "true" ]; then
    ...
    export BYFN_CA1_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/org1.ryan.blockchain.com/ca && ls *_sk)
    export BYFN_CA2_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/org2.ryan.blockchain.com/ca && ls *_sk)
  fi
...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.ryan.blockchain.com | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.ryan.blockchain.com
    docker cp -a orderer.ryan.blockchain.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.ryan.blockchain.com
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.ryan.blockchain.com

    for PEER in peer0.org1.ryan.blockchain.com peer1.org1.ryan.blockchain.com peer0.org2.ryan.blockchain.com peer1.org2.ryan.blockchain.com; do
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/org1.ryan.blockchain.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/org2.ryan.blockchain.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
# (``peer0.org1.ryan.blockchain.com`` & ``peer0.org2.ryan.blockchain.com``).  Secondly, we point to
