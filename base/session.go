...
// ClientProvider creates and returns context client which has all the necessary providers
func (sess *Session) ClientProvider() contextApi.ClientProvider {
	return sess.fabsdk.Context(fabsdk.WithUser(sess.Options.Username), fabsdk.WithOrg(sess.Options.Organization))
}

// ChannelProvider creates and returns channel context
func (sess *Session) ChannelProvider(channel string) contextApi.ChannelProvider {
...
func (sess *Session) DiscoverLocalPeers() ([]fab.Peer, error) {
	orgUserContext := sess.ClientProvider()
